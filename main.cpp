	...
	while (!glfwWindowShouldClose(window))
	{
		...
		case program_modes::printing:
		{
			...
			if (output_gcode)
			{
				...
				f.open("digital-daguerreotype.nc", fstream::out);
				f << std::fixed;
				f << std::setprecision(4);
				f << "; generated by digital-dagerreotype" << endl;
				...
				int error = system("gcode-cli digital-daguerreotype.nc /dev/ttyUSB0,b115200");
				if (error)
