	...
	while (!glfwWindowShouldClose(window))
	{
		...
		case program_modes::printing:
		{
			...
			if (output_gcode)
			{
				...
				// output file header
				f.open("digital-daguerreotype.gcode", fstream::out);
				f << std::fixed;
				f << std::setprecision(4);
				f << "(gcode generated by digital-dagerreotype)" << endl;
				f << "G21" << endl;		// programming in mm
				f << "G1 F1000" << endl;// set a feed rate because grbl requires it
				f << "G90" << endl;		// use absolute coordinates
				f << "G01" << endl;		// linear interpolation between startand end points
				f << "G1 Z0" << endl;	// lift the pen

				// output a valid .gcode file for printing
				for (Path::iterator i = tsp.begin(); i != tsp.end(); ++i)
				{
					float x = (float)(*i).x * easWidthMM / easWidthPixels;
					float y = (float)(*i).y * easHeightMM / easHeightPixels;

					// output each point as the next position to move to
				...
				int error = system("gcode-cli digital-daguerreotype.gcode /dev/ttyUSB0,b115200");
				if (error)
					throw std::runtime_error("error spawning gcode-cli");
